...
The `ajantv2.ko` module shall be compiled with GPU Direct RDMA
enabled (`AJA_RDMA=1`). This allows zero-copy DMA between the
Corvid44 and NVIDIA GPU memory, bypassing system RAM.
...
The NVIDIA header version must match the driver version shipped by
the base image. The build stage shall detect the installed NVIDIA
driver version and fetch `nv-p2p.h` from the corresponding
open-gpu-kernel-modules tag.

The `nvidia-peermem` kernel module (shipped by the base image's
`kmod-nvidia-open` package) must be loaded at runtime for RDMA to
function. The image shall include a `modules-load.d` entry for
`nvidia-peermem` alongside the existing `ajantv2` entry.
